...
class Rh_refeicao extends MY_Controller{
    ...
    public function __construct(){
        ...
        $this->dados['modulo_menu'] = array('Importados' => 'import_refeicao',
                                            ...
                                            'Refeição-IdSecure' => 'sync_idsecure',
                                            'teste' => 'servico'
                                        );
    ...
    function sync_idsecure($page="0", $sync="0"){
        $post= $_POST[sync];

        if($sync === "1" || json_decode($post) === true) {
            ...
            if($this->idsecure->authenticate()){
                $retorno = $this->idsecure->callRequestWithOutData('api/util/SyncAll', true);
                if ($retorno === "") {
                    $this->dados['retorno'] = "Susesso";
                } else {
                    $this->dados["retorno"] = "Não foi possível se conectar";
                }
            }
        }
        ...
        // 
        $this->render('sync_idsecure');
    }

    function remove_group(){
        $user_post = $_POST[user];
        $ref_post = $_POST[refhr];
            ...
            if($this->idsecure->authenticate()){
                // if($this->idclass->authenticate()){
                    ...
                    // foreach ($afd as $key => $value) {
                        ...
                        $user = $this->idsecure->searchUser(substr($user_post, 1));
                        if($user){
                            ...
                            foreach ( $user->groups as $key => $value) {
                                ...
                                $value == Rh_refeicao::GRUPO00 ){
                                    ...
                                    foreach ( $user->groupsList as $key => $value) {
                                        if($value->id == Rh_refeicao::GRUPO12 ||
                                        $value->id == Rh_refeicao::GRUPO19 ||
                                        $value->id == Rh_refeicao::GRUPO22 ||
                                        $value->id == Rh_refeicao::GRUPO00 ){
                                            unset($user->groupsList[$key]);
                                            foreach ( $user->userGroupsList as $key => $value) {
                                                if($value->id == Rh_refeicao::GRUPO12 ||
                                                $value->id == Rh_refeicao::GRUPO19 ||
                                                $value->id == Rh_refeicao::GRUPO22 ||
                                                $value->id == Rh_refeicao::GRUPO00 ){
                            ...
                            $retorno = $this->idsecure->setUserGroup($user);
                            $retornojson = json_decode($retorno);
                            if ($retorno) {
                                if ($retornojson->code === 200) {
                                    $retorno = $retorno;
                                } else {
                                    $retorno = null;
                                }
    ...
    function import_manual($page = 0){
        ...
        $PIS = json_decode($get[filtro][like][NOME]);
        if (strlen($PIS) >= 10){
            if ($ref > 0){
                $equipamento = $this->biometria_equipamento_model->get_all(array('TIPO'=>'IDSECURE'));
                $constructIdSecure = ["ip" => $equipamento[0]->IP, "port" => $equipamento[0]->PORTA, "user" => $equipamento[0]->USUARIO, "password" => $equipamento[0]->SENHA,"protocol"=>$equipamento[0]->PROTOCOLO];
