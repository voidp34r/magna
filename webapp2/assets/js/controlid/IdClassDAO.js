...
//var repurl = 'https://192.168.0.146/'; // debug only
var repurl = '';
...
function getAFD(cTitle,data) {
    ...
    LoadObject('get_afd', data, function (result) {
//        md.setContent('Download concluído');
	...
	for (var i = line_end-1; i >= 1; i--) {
            ...
            if (resultView.getInt8(i) == 10 && resultView.getInt8(i-1) == 13) {
                // Achou nome do arquivo
                i--;
                if (filename == null) {
                    filename = String.fromCharCode.apply(null, new Uint8Array(result.slice(i+2, line_end)));
                    line_end = i+1;
                    console.log(filename);
                }
                    // Achou header
                else {
                    header = new Uint8Array(result.slice(i+2, line_end));
                    line_end = i+1;
                    break;
                }
            }
        ...
        //var superAFD = header + '\r\n' + result.substring(0, posFilename).substring(0, posHeader);
        ...
        try{
            ...
            fixedstring=decodeURIComponent(escape(superAFD));
        }catch(e){
            // If it isn't, an error will be thrown, and we can asume that we have an ISO string.
            fixedstring=superAFD;
        }*/
        //var superAFD = result.afd.header + '\r\n' + result.afd.data + result.afd.trailer + '\r\n' +result.afd.signature;

        var textFileAsBlob = header[0] != 13    ? new Blob([header, result.slice(0,line_end-1)],  { encoding:"ISO-8859-1", type: "text/plain; charset=ISO-8859-1" })
                                                : new Blob([result.slice(0,line_end-1)],  { encoding:"ISO-8859-1", type: "text/plain; charset=ISO-8859-1" });
	...
	function checkJsonError(jData){
		for(var key in jData){
			if(Array.isArray(jData[key])){
				...
				else if(typeof jData[key] === 'object' && 'error' in jData[key])
		...
		SendJSON(commad, jsonData, function(data){
			if(
				checkSession(
					data,
					dvBlock,
					function(newCount){ SaveObject(title, commad, jsonData, finishCallback, newCount, dvBlock);},
					count
				)
			){
				unblockPage(dvBlock, function(){ if(finishCallback) finishCallback() });
...
function LoadObject(command, params, finishCallback, ajaxParams, count, dvBlock, rawReturnData, dontFadeOut){
	if(!dvBlock)
		dvBlock = blockPage();
	if(getCookie('session'))
	...
	SendJSON(command, params, function(result){
		...
		if(
			checkSession(
				result,
				dvBlock,
				function(newCount) { LoadObject(command, params, finishCallback, ajaxParams, newCount, dvBlock); },
				count
			)
		){
			unblockPage(dvBlock, function(){ if(finishCallback) finishCallback(result) }, dontFadeOut);
		}else if(result && result.error && result.error.indexOf('Invalid session') == -1){
			var md = new Modal();
			md.setContent('Ocorreu um erro:<br />');
...
function RemoveObject(command, params, finishCallback, ajaxParams, count, dvBlock){
	if(!dvBlock)
	...
	SendJSON(command, params, function(result){
		if(
			...
			unblockPage(dvBlock, function(){ if(finishCallback) finishCallback(result) });
		}else if(result && result.error && result.error.indexOf('Invalid session') == -1){
			var md = new Modal();
			md.setContent('Ocorreu um erro:<br />');
			md.addContent(result.error);
