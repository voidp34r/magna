...
class MY_Model extends CI_Model
{
    ...
    public $pagination_disabled;
    protected $pagination_where = array();
    ...
    public function __construct ()
    {
        ...
        $this->pagination_arrows = array('&lt;', '&gt;');
        $this->pagination_disabled = '<li class="disabled">';
        $this->pagination_active = '<li class="active">';
...
//                $id = $this->_database->insert_id();
                $return = $this->trigger('after_create', !empty($id) ? $id : NULL);
                return $return;
            }
            return FALSE;
        }
    ...
    public function _get_next_id ()
    {
        $table = trim(substr($this->table, 0, 26));
        $this->db->select($table . '_SEQ.NEXTVAL AS NEXTID', FALSE);
        $this->db->from('DUAL');
        $query = $this->db->get();
        $row = $query->row();
        return $row->NEXTID;
    ...
    public function as_dropdown ($field = NULL)
    {
        ...
        $this->_database->order_by($field, 'ASC');
        return $this;
    }
    ...
    function _impode_query_string ($uri_array)
    {
    ...
    public function paginate ($rows_per_page, $total_rows = NULL, $page_number = 1,$where = NULL)
    {
        ...
        {
            $uri_array[$segments] = '';
            $uri_string = $this->_impode_query_string($uri_array);
            $this->previous_page = $this->pagination_disabled . anchor($uri_string, $this->pagination_arrows[0]) . $this->pagination_delimiters[1];
        }
        else
        {
        ...
        {
            $uri_array[$segments] = $page;
            $uri_string = $this->_impode_query_string($uri_array);
            $this->next_page = $this->pagination_disabled . anchor($uri_string, $this->pagination_arrows[1]) . $this->pagination_delimiters[1];
        }
        else
        {
        ...
        if (isset($total_rows))
        {
            if ($total_rows != 0)
            {
                ...
                $page_ini = 1;
                $page_end = $number_of_pages;
                if ($number_of_pages > 10)
                {
                    $page_ini = $page_number - 5;
                    $page_end = $page_number + 5;
                }
                if ($page_ini < 1)
                {
                    $page_end += abs($page_ini);
                    $page_ini = 1;
                }
                if ($page_end > $number_of_pages)
                {
                    $page_end = $number_of_pages;
                }
                $links = '';
                if ($page_number != 1)
                {
                    $uri_array[$segments] = 1;
                    $uri_string = $this->_impode_query_string($uri_array);
                    $links .= $this->pagination_delimiters[0] . anchor($uri_string, '<<', 'title="Primeira"') . $this->pagination_delimiters[1];
                }
//                $links .= $this->previous_page;
                for ($i = $page_ini; $i <= $page_end; $i++)
                {
                    unset($uri_array[$segments]);
                    $uri_array[$segments] = $i;
                    $uri_string = $this->_impode_query_string($uri_array);
                    $links .= ($i == $page) ? $this->pagination_active : $this->pagination_delimiters[0];
                    $links .= anchor($uri_string, $i);
                    $links .= $this->pagination_delimiters[1];
                }
//                $links .= $this->next_page;
                if ($page_number != $number_of_pages)
                {
                    $uri_array[$segments] = $number_of_pages;
                    $uri_string = $this->_impode_query_string($uri_array);
                    $links .= $this->pagination_delimiters[0] . anchor($uri_string, '>>', 'title="Última"') . $this->pagination_delimiters[1];
                }
                $this->all_pages = ($number_of_pages > 1) ? $links : '';
            }
            else
            {
        ...
        else
        {
            ...
            if(!empty($this->pagination_where)){
                foreach($this->pagination_where as $value){
                    $this->where($value->param1,$value->param2,$value->param3,$value->param4,$value->param5);
                }
            }
    ...
    /**
     * Este método adiciona as condicionais para um array que será utilizado para realizar o where posteriormente na paginação
     * Para saber o que cada parametro significa consulte o método "where" desta classe(My_Model.php); 
     */
    public function set_where_pagination($param1 = NULL,$param2 = NULL,$param3 = NULL,$param4 = FALSE,$param5 = FALSE,$param6 = FALSE){ 
        $where = new StdClass();
        $where->param1 = $param1;
        $where->param2 = $param2;
        $where->param3 = $param3;
        ...
        $where->param5 = $param5;
        $where->param6 = $param6;
        array_push($this->pagination_where,$where);
        $this->where($where->param1,$where->param2,$where->param3,$where->param4,$where->param5,$where->param6);
        return $this;
    }
