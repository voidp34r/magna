...
Class IdSecure {
  ...
  private $protocol;
    ...
    public function callRequestWithOutData($method,$useSession = false){
      $ch = curl_init();
      if($useSession){
          $header = array("content-type: application/json", "Authorization: Bearer ".$this->getSession());
      }else{
    ...
    public function callRequestPost($method,$postData,$useSession = false){
        $ch = curl_init();
        if($useSession){
            $header = array("content-type: application/json", "Authorization: Bearer ".$this->getSession());
        }else{
    ...
    public function getGroupsUser($id = null, $groupCod = null){
        $group = [];
        if($id) {
    ...
    public function getAllGroups($id = null){
        $group = [];
        if($result = $this->callRequest(false,'api/group','', true)){
            if(json_decode($result)->data){
                $group = json_decode($result)->data; 
            }
        }
        return $group;
      }
    ...
    public function setUserGroup($user){
        $array = [];
      if($result = $this->callRequestPut('api/util/savecustomfields',$array, true)){
          return $result = $this->callRequestPost('api/user/',array($user), true);
      }else{
          return $result = $this->callRequestPost('api/user/',array($user), true);
      }
    }
