...
Class IdSecure
{
 ...
 //Fecha a conexão
    ...
    public function syncAllIdSecure() {
        $retorno = $this->callRequestWithOutData('api/util/SyncAll', true);
        if($retorno != ""){
            $retorno = "Erro ai tentar sincronizar!";
            return $retorno;
        } else {
            if ($retorno === "") {
                $retorno = "Sincronização relizada com sucesso!";
                return $retorno;
            } else {
                $retorno = "verifique o fonte para saber o retorno desta requisição!";
                return $retorno;
            }
        }
