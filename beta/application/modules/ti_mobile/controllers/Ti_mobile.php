...
class Ti_mobile extends MY_Controller{
        ...
        if ($this->form_validation->run()){
            ...
            if($this->usuario_portaria_model->update($post, $id)){
                ...
                if($this->usuario_model->update($update_data)){
                   $this->redirect('ti_mobile/listar_usuario', 'sucesso', 'Usuário alterado com sucesso!'); 
                }else{
    ...
    public function excluir_usuario($id){
        ...
        try
        {
            ...
            $this->redirect('ti_mobile/listar_usuario', 'sucesso', 'Usuário excluido com sucesso!');
    ...
    **/
            ...
            if(!$usuario){
               ...
               if($idUsuPor){
                   ...
                   if($this->usuario_model->insert($insert_data)){
                       $this->redirect('ti_mobile/listar_usuario', 'sucesso', 'Usuário cadastrado com sucesso!'); 
                   }else{
                ...
                $html = "Já existe um usuário com este login:<br>
                         <a href='ti_mobile/editar_usuario/{$usuario->ID}'>{$usuario->USUARIO_NOME}</a>
                         ";
    ...
    public function salvar_foto_config(){
        ...
        $id = $this->portaria_foto_config_model->insert($this->input->post());
        echo json_encode($id ? true : false);
    }
    
    public function atualizar_foto_config($id = null){
        
        $idRet = $this->portaria_foto_config_model->update($this->input->post(),$id);
        echo json_encode($idRet ? true : false);
    }    
    
    public function remover_foto_config($id = null){
        
        $idRet = $this->portaria_foto_config_model->delete(array('ID' => $id));
        echo json_encode($idRet ? true : false);
    }
    ...
    public function fotos_sequencia($id){
        
        $obj =  $this->portaria_foto_config_model
                     ->where('FOTO_CATEGORIA_ID',$id)
    ...
    public function atualiza_checklist_sequencia($categoria){ 
        
        $tpUpdate = $categoria == 'true' ? true : false;
        ...
        foreach ($obj[arr] as $key => $value) {
            ...
            if($tpUpdate){
                ...
                $ret  = $this->portaria_foto_config_model->update(array('NRSEQUENCIA' => $objUpd->SEQUENCIA),$objUpd->ID);
            }
